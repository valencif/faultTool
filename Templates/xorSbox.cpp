...
void xorSbox::setNumTraces(int FSA, int FSAT){
	numTracesFSA=FSA;
	numTracesFSAT=FSAT;
}
...
xorSbox::xorSbox() {
	...
	__ModleType="xorSbox";
	numInputs=1;
	inputContainer= new int[numInputs];
	keyContainer=0;
	fixed_key=0;
...
int xorSbox::setInput(int index, int value){
	if(index<0 || index>1){
		cout<<"Check input index"<<endl;
		return -1;
	}

	inputContainer[index]=value;
	return 0;
}

int xorSbox::setKey( int value){
	keyContainer=value;
	return 0;
}
int xorSbox::getOuput(){
    
    int sboxin;
    sboxin = (inputContainer[0]  ^ keyContainer) %256;
	return S_BOX[ sboxin ];
}

int xorSbox::createStimuliFSA(string &filename){

	ofstream fileFSA;

	if(fileExists(filename)){
		display.print(VIEW_ERROR)<<"Stimuli FSA file already exists and will not be updated"<<endl;
		return STIMULI_EXISTS;
	}

	fileFSA.open(filename.c_str(), ios::out );

	if ( fileFSA.fail() ){
		return STIMULI_NOT_CREATED;
	}

	// See if RAND_MAX is big enough in the compiling platform
	fixed_key= rand() % 256;
	keyContainer=fixed_key;

	for (int i=0; i< numTracesFSA; i++){
		inputContainer[0]=rand() % 256;
		fileFSA<<inputContainer[0]<<" "<<keyContainer<<" "<<getOuput()<<"\n";
	}

	fileFSA.close();

	return STIMULI_CREATED;

}
 int xorSbox::createStimuliFSAT(string &filename){
	ofstream fileFSAT;

	if(fileExists(filename)){
		display.print(VIEW_ERROR)<<"Stimuli FSA Template file already exists and will not be updated"<<endl;
		return STIMULI_EXISTS;
	}

	fileFSAT.open(filename.c_str(), ios::out );
	...
	// See if RAND_MAX is big enough in the compiling platform
	for (int i=0; i< numTracesFSAT; i++){
		keyContainer=rand() % 256;
		inputContainer[0]=rand() % 256;
		fileFSAT<<inputContainer[0]<<" "<<keyContainer<<" "<<getOuput()<<"\n";
	}
 ...
 int xorSbox::getFixedkey(){
	 return fixed_key;
 }

 int xorSbox::getMaxIntermediate(){
	 return 256;
 }

 int xorSbox::getOuputPos(){
	 return 3;

 }

 int xorSbox::getMaxKey(){
	 return 256;
 }

 void xorSbox::loadModelInfo(string stimuliFSA, string stimuliFSAT){

	 FFManager &folderfiles = FFManager::instance();
	 int corrKey;
	 ifstream file;

	 file.open(stimuliFSA, ios::in);
	 file >> corrKey; // Skip first value
	 file >> corrKey;
